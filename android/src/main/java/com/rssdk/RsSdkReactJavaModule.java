...
import com.rssdk.jni.JniRsSdkCallback;
 ...
 */
@ReactModule(name = RsSdkReactJavaModule.TAG)
public class RsSdkReactJavaModule extends ReactContextBaseJavaModule implements JniRsSdkCallback {
    public static final String TAG = RsSdkReactJavaModule.class.getSimpleName();
    ...
    public void rs_device_login(String dev_long, String param, Promise promise) {
        int result = -1;
        try {
            is_close_type_received = false;
            login_devices_callback_params_list.clear();
            login_devices_future = new CompletableFuture<>();
            result = JniRsSdkWrapper.rs_device_login(Long.parseLong(dev_long), param, RsSdkReactJavaModule.this);

            login_devices_future.get(10, TimeUnit.SECONDS);
            promise.resolve(prepare_login_response(result));
    ...
    public void connction_callback(String param) {
        ...
        switch (type) {
            ...
            case "force close brower":
                break;
            case "close":
                if (!is_close_type_received) {
                    is_close_type_received = true;
                    break;
                }
...
//        Log.e(TAG, "Called `alarm_callback` with type: " + alarm_type + " and param: " + param);
     ...
     */
    @ReactMethod
    public void rs_start_preview(String dev, String param, Promise promise) {
        try {
            long preview_sess = JniRsSdkWrapper.rs_start_preview(Long.parseLong(dev), param, RsSdkReactJavaModule.this);
            Log.d(TAG, "Called `rs_start_preview` with result: " + preview_sess);
            promise.resolve(Long.toString(preview_sess));
        } catch (Exception e) {
